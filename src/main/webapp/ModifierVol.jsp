...
<h1>Modifier informations Vol</h1>
     <hr/>
   <div class="col-md-6">
     ...
     <form:form method="post" action="" modelAttribute="vol">
       ...
       <div class="form-group">
         ...
         <form:select path="aeroportDepart.id" class="form-control">
         	<c:forEach items="${requestScope.aeros }" var="item">
         	   <c:choose>
         	     <c:when test="${item.id == vol.aeroportDepart.id }">
       ...
       <div class="form-group">
         ...
         <form:select path="aeroportArrivee.id" class="form-control">
         	<c:forEach items="${requestScope.aeros }" var="item">
         	   <c:choose>
         	     <c:when test="${item.id == vol.aeroportArrivee.id }">
         	        <option value="${item.id}" selected>${item.nom }</option>
         	     </c:when>
         	     <c:otherwise>
         	        <option value="${item.id}">${item.nom }</option>
       ...
       <div class="form-group">
         <label>Avion</label>
         <form:select path="avion" class="form-control">
           <c:forEach items="${requestScope.avions }" var="item">
              <c:choose>
                <c:when test="${item.id == requestScope.vol.avion.id }">
       ...
       <div class="col-md-12">
       <br/><br/><br/>
       	<input type="submit" value="Enregistrer Modifications" class="btn btn-danger"/>
       </div>
   ...
   <div class="col-md-5">
     <c:choose>
      <c:when test="${requestScope.vol.escales.size()>0}">
         <h2>Liste Escales</h2>
         <br/>
         <table class="table">
           <c:forEach items="${requestScope.vol.escales}" var="item">
             <tr id="l${item.id }">
               <td>${item.aeroport.nom }</td>
               <td> <button class="btn btn-danger deleter" id="${item.id }">Supprimer</button></td>
             </tr>
           </c:forEach>
         </table>
      </c:when>
            ...
            $(function () {
				...
				*/
 				...
 				$('.deleter').click(function(){
 					var res = confirm('Etês vous sure de vouloir supprimer cet Escale !');
 				    if(res){
 				    	var idClient = this.id;
 						$.get('/agent/supprimer-escale?id='+idClient,function(data){
 							if(data == 'OK')
 								$('#l'+idClient).remove();
 							else
